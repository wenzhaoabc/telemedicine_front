    ...
    <div>
        <div class="wrapper">
            <div class="card-inner">
                ...
                <p> <span v-if="(doctorInfo.gender == 1)">男</span> <span v-if="(doctorInfo.gender == 0)">女</span> <span
                        style="margin-left:10px">{{ doctorInfo.age }} </span> </p>
                <p>
                    {{ doctorInfo.hospital }}
                </p>
                ...
                <div class="doctor-service">
                    <a-button @click="openModel" class="infos">
                        <template #icon>
                            <icon-language />
                        </template>
                        图文问诊
                    </a-button>
                    <a-button @click="handleClick2" class="infos">
                        <template #icon>
                            <icon-video-camera />
                        </template>
                        视频问诊
        ...
        <a-modal v-model:visible="visible" @ok="handleOk" @cancel="handleCancel" :footer=false>
            ...
            <div>
                <a-textarea v-model="problem" :auto-size="{ minRows: 2, maxRows: 5 }" placeholder="请简单描述病症状况，不要超过100个字"
                    :max-length="90" allow-clear />
            </div>
            <div style="width:100%">
                <div v-if="!idle" style="color:red;text-align:center;margin-top:10px">该医生当前不是空闲状态，无法咨询，请等等再来咨询或者换一个医生
                </div>
...
function goDoctorInfo() {
    router.push({ path: `/doctorInfo/${String(doctorInfo.value.actorId)}` })
}
</script>
